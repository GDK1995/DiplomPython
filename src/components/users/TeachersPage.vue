<template>
  <div class="user">
    <div class="user_head">
      <p class="heading_xl_h4">{{$t('user_1')}}</p>
      <button v-if="!isTableVisible" @click="visibilityTable" class="medium_s">{{$t('add_teacher')}}</button>
    </div>
    ...
    <form class="user_create" v-else @submit.prevent="visibilityTable">
      ...
      <button @click="registrate">Сохранить</button>
    </form>
  </div>
</template>
<script>
